<div align="center">
  <img src="https://cdn.bili33.top/gh/GDUTMeow/.github/pic/cat-in-bin.jpg" height="10%" width="10%">
  <h1>GDUTMeow</h1>
</div>

> [!important]
>
> 起这个名字是因为工一有一只白猫异色瞳猫猫，超可爱~~~
>
> <div align="center">
> <img src="https://cdn.bili33.top/gh/GDUTMeow/.github/pic/cat.jpg" height=50% width=50%>
> </div>

本仓库是我拿来存放我在窝工（[广东工业大学](https://www.gdut.edu.cn/)）做的各种作业/设计的，因为想跟我的个人账户分开，所以就创建了这样的一个组织

如果你需要找的是 CTF 题目，你需要前往 [@GDUTMeow-Challenges](https://github.com/GDUTMeow-Challenges) 查看

下面是各种联系方式

- Telegram 私信bot：[@PaffCreamPMBot](https://t.me/PaffCreamPMBot)

- Telegram 频道：[@PaffChannel](https://t.me/PaffChannel)
- Telegram 群组：[@DohnaNyan](https://t.me/DohnaNyan)
- 邮箱：[GamerNoTitle@outlook.com](mailto:GamerNoTitle@outlook.com)

## 学科归档类

### 大一上

- [计算机学院 C 语言程序设计 Anyview 题解](https://github.com/GDUTMeow/Anyview-Programming2024-Reference)

### 大一下

- [Python 数值分析实验及作业题解](https://github.com/GDUTMeow/NumericalAnalysisWorks)

### 大二上

- [数字逻辑与电路设计（EDA）作业及实验题解](https://github.com/GDUTMeow/EDA2025-Libero-Experiment)
- [数据结构 Anyview 题解](https://github.com/GDUTMeow/Anyview-DataStructure2025-Reference)
- [数据结构课程实验（有向图）](https://github.com/GDUTMeow/Mat-Based-Directed-Graph-Implementation)

### 大二下

- [Java 程序设计实验题解](https://github.com/GDUTMeow/JavaProgrammingExperiment)
- [操作系统实验（华为基座类）题解](https://github.com/GDUTMeow/OperationgSystemExperiment)
- [计算机组成原理 Quartus 实验题解](https://github.com/GDUTMeow/ComputerOrganizationExperiment)

### 大三上

- [密码学基础个人作业留档](https://github.com/GDUTMeow/CryptographyBasic)
- [网络空间安全概论个人作业留档](https://github.com/GDUTMeow/CyberSecurityBasis)
- [计算机系统体系结构个人作业留档](https://github.com/GDUTMeow/ComputerSystemArchitecture)
- [数据库系统个人作业留档](https://github.com/GDUTMeow/DatabaseSystem)

## 工具类

- [GDUT 体育/选修课抢课脚本](https://github.com/GDUTMeow/gdut-course-grabber)
- [Welearn 刷课挂时长工具](https://github.com/GDUTMeow/WelearnBrainBurst)
- [Classviva 题目 HTML 源码复制油猴脚本](https://github.com/GDUTMeow/ClassViva-Copier)
- [蕴瑜课堂视频课自动通过在线检测油猴脚本](https://github.com/GDUTMeow/YunyuImListening)
- 蕴瑜课堂自动刷视频课脚本（未公开）
- [适用于本校的 Openwrt 路由器脚本](https://github.com/GDUTMeow/WrtScript)
- [GDUT 无线网络自动登录脚本](https://github.com/GDUTMeow/GDUT-Wlan-Login)
- [总务处报修绕过微信浏览器检测](https://github.com/GDUTMeow/NoWechatOnPropertyWeb)

## 个人设计类

- [C 语言程序设计课设 - 数学口算系统](https://github.com/GDUTMeow/MentalArithmeticApp)
- [数据结构课程设计 - 旅游区导览系统](https://github.com/GDUTMeow/ScenicPathfinder)
- [计算机网络课程设计 - 简单的网络嗅探器]()
